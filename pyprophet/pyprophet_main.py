...
from pyprophet.std_logger import logging
import sys
import random
import time
import warnings
...
from pyprophet.pyprophet_fun import PyProphet
from pyprophet.config import CONFIG, set_pandas_print_options
from pyprophet.report import save_report, export_mayu, mayu_cols

from pyprophet.main_helpers import (parse_cmdline, create_pathes, check_if_any_exists)
...
class PyProphetRunner(object):
    ...
    def run(self, return_df=False):
        ...
        if return_df:
            return result.scored_tables[0].df
...
def _main(args, return_df=False):
    ...
    if learning_mode:
        if out_of_core:
            ...
            return PyProphetLearner(pathes, prefix, merge_results, delim_in, delim_out).run(return_df)
